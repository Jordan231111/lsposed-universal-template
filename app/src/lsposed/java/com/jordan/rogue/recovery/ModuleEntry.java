...
public final class ModuleEntry extends XposedModule {
    ...
    private synchronized void installNativeHookToggleListener(Context appContext) {
        ...
        FeatureRegistry.addListener(key -> {
            ...
            if (nativeHooksStarted && FeatureRegistry.KEY_GAME_SPEED_MULTIPLIER.equals(key)) {
                // Slider drag fires this listener on every onProgressChanged tick. Take the fast
                // path so the IL2CPP speedUpValue field is updated on the same frame instead of
                // logging the full feature snapshot.
                NativeBridge.pushGameSpeedMultiplier(
                        FeatureRegistry.getFloat(FeatureRegistry.KEY_GAME_SPEED_MULTIPLIER));
            } else if (nativeHooksStarted && isRecoveredFeatureKey(key)) {
    ...
    private static boolean isRecoveredFeatureKey(String key) {
        return FeatureRegistry.KEY_DAMAGE_MULTIPLIER.equals(key)
                ...
                || FeatureRegistry.KEY_FORGE_BACKUP_SUCCESS.equals(key)
                || FeatureRegistry.KEY_GAME_SPEED_MULTIPLIER.equals(key);
    }
