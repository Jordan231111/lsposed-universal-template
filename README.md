...
- Modern `libxposed` API 101 entry point (`io.github.libxposed:api:101.0.1`, `compileOnly`)
  for the `lsposed` flavor, plus a classic Xposed API 82 entry for the `lspatch` flavor.
...
- `FeatureRegistry` — runtime feature flags (bool/float) with best-effort persistence,
  live-updating overlay toggles, and native-state auto-sync for every feature key.
...
- Integrity-bypass scaffold (`com.jordan.rogue.recovery.protection.IntegrityBypass`) with
  ...
  IL2CPP hooks for the cloud-save path. The key hook injects the app's Google cloud
  project number into `IntegrityTokenRequest`; the optional forge layer rewrites
  `IssueBackupKey` / `VerifyBackupKey` closure output after the original handler runs.
  Lower PlayFab transport hooks log the serialized request and response so a missing
  transfer code can be distinguished from a UI-only failure. See
...
This branch's native code waits for `libil2cpp.so`, resolves the ELF load base from
`/proc/self/maps`, primes persisted feature state before hook install, and installs IL2CPP
bootstrap hooks on exported runtime entry points. The recovered Rogue hooks are then resolved
from IL2CPP metadata by assembly/class/method name, with feature-critical managed fields resolved
by field name as well. Game-speed state is synced early but applied only from safe IL2CPP
callbacks (`SetupNewMode`, `ReflectSpeedUp`, `Time.set_timeScale`, or PlayFab result callbacks).
...
metadata recovery, runtime metadata lookup, value-type ABI checks, delayed library loading, and
settings-bridge issues that also apply to other native-heavy engines. Treat it as a playbook for
a branch that targets one app, not as default template behavior.
...
- `lsposed`: side-loaded LSPosed/Vector module using libxposed API 101.
- `lspatch`: classic Xposed entry under `app/src/lspatch/` for wrapping a target APK with
  LSPatch. It uses the same native hook library and loads persisted `.rt_state` values from
  the target app's files directory, but it does not show the floating overlay.
